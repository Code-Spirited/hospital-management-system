...
export const IPDProvider = ({ children }) => {
  ...
  // Appends one treatment record onto an admission's history. An admission
  // accumulates many of these over a stay — unlike OPD's one-time
  // Consultation record. The record's id/timestamp are generated by the
  // caller (TreatmentRecords.jsx) using the shared generateId utility, the
  // same pattern already used for admissions/appointments — keeping this
  // function itself a plain, pure state update.
  const addTreatmentRecord = useCallback((admissionId, record) => {
    setAdmissions((prev) =>
      prev.map((a) =>
        a.id === admissionId
          ? { ...a, treatmentRecords: [...(a.treatmentRecords || []), record] }
          : a,
      ),
    );
  }, []);
